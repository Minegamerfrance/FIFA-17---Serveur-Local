...
const UTIL_CMD: Record<number, string> = {
  1: "fetchClientConfig",
  2: "ping",
  3: "setClientData",
  4: "localizeStrings",
...
const AUTH_CMD: Record<number, string> = {
...
function cmdName(component: number, command: number): string {
  if (component === Component.Util) return UTIL_CMD[command] ?? `util_${command}`;
  if (component === Component.Authentication) return AUTH_CMD[command] ?? `auth_${command}`;
  if (component === Component.UserSessions) return `userSessions_${command}`;
  return `${component}/${command}`;
}

function tryDecodeTdf(payload: Buffer): { ok: true; fields: Record<string, unknown> } | { ok: false; error: string } {
  if (payload.length === 0) return { ok: true, fields: {} };
  try {
...
export function traceBlazeRequestIn(rawPacket: Buffer, pkt: NonNullable<ReturnType<typeof parseBlazePacket>>): void {
  ...
  log(
    "info",
    "blaze-trace",
    `★★★ REQ ${name} component=${pkt.component} command=${pkt.command} msgType=${pkt.msgType} msgNum=${pkt.msgNum} error=${pkt.error} options=${pkt.options} style=${pkt.headerStyle} packetLen=${rawPacket.length} payloadLen=${pkt.payload.length}`,
  );
...
export function traceBlazeReplyOut(
  reply: Buffer,
  meta: { handler: string; reqComponent: number; reqCommand: number },
): void {
  ...
  log(
    "info",
    "blaze-trace",
    `★★★ REP ${name} handler=${meta.handler} len=${reply.length}` +
      (parsed
...
export function utilCommandName(command: number): string {
