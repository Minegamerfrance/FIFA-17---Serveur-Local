...
export function handleBlazeConnection(
  ...
  const notifyDelayMs = Math.max(
    0,
    Number.parseInt(process.env.AUTH_NOTIFY_DELAY_MS ?? "0", 10) || 0,
  );
  ...
  if (isProtoSsl) {
    remoteIp = (socket as ProtoSslSocket).socket.remoteAddress || "";
    remotePort = (socket as ProtoSslSocket).socket.remotePort || 0;
  } else {
    remoteIp = (socket as net.Socket).remoteAddress || "";
    remotePort = (socket as net.Socket).remotePort || 0;
  }
    ...
    for (;;) {
      ...
      if (!firstAppLogged) {
        ...
        log(
          "info",
          "blaze",
          `★★★ BLAZE_APP_FIRST IN component=${pkt.component} command=${pkt.command} msgType=${pkt.msgType} options=${pkt.options} msgNum=${pkt.msgNum} style=${pkt.headerStyle} payloadLen=${pkt.payload.length} hex=${head.toString("hex")}`,
        );
      } else {
        log(
          "info",
          "blaze",
          `IN #${appInCount} component=${pkt.component} command=${pkt.command} msgType=${pkt.msgType} options=${pkt.options} style=${pkt.headerStyle} payloadLen=${pkt.payload.length} hex=${head.toString("hex")}`,
        );
      }
      ...
      routeBlazeRequest(pkt, session, (reply, handler = "unknown") => {
        ...
        const sendReply = () => {
          ...
          log(
            "info",
            "blaze",
            `OUT #${appOutCount} reply len=${reply.length} handler=${handler} hex=${reply.subarray(0, Math.min(64, reply.length)).toString("hex")}`,
          );
          if (
            pkt.component === Component.Util ||
            pkt.component === Component.Authentication ||
          ...
          if (isProtoSsl) {
            (socket as ProtoSslSocket).writeApp(reply);
            ...
            if (
              ...
              setTimeout(() => {
                try {
                  (socket as ProtoSslSocket).writeApp(Buffer.alloc(0));
                  log("info", "blaze", "Auth/10 ProtoSSL empty-record flush sent");
                } catch (err) {
                  log("warn", "blaze", `Auth/10 ProtoSSL flush failed: ${(err as Error).message}`);
                }
              }, 25);
              ...
              if (notifyDelayMs >= 1500) {
                ...
                setTimeout(() => {
                  try {
                    ...
                    (socket as ProtoSslSocket).writeApp(wake);
                    log(
                      "info",
                      "blaze",
                      `Auth/10 neutral Blaze wake UserSessions/32767 sent after ${wakeDelayMs}ms`,
                    );
                    setTimeout(() => {
                      try {
                        (socket as ProtoSslSocket).writeApp(Buffer.alloc(0));
                        ...
                        log(
                          "warn",
                          "blaze",
                          `Auth/10 neutral wake flush failed: ${(err as Error).message}`,
                        );
                      }
                    }, 25);
                  } catch (err) {
                    log(
                      "warn",
                      "blaze",
                      `Auth/10 neutral Blaze wake failed: ${(err as Error).message}`,
                    );
                  }
                }, wakeDelayMs);
              }
            ...
            (socket as net.Socket).write(reply);
          }
        };
        ...
        const isAuthLoginNotification =
          pkt.component === Component.Authentication && pkt.command === 0x0a;
        if (
          notifyDelayMs > 0 &&
          replyMsgType === MsgType.Notification &&
          ...
          log(
            "info",
            "blaze",
            `delaying notification OUT by ${notifyDelayMs}ms handler=${handler}`,
          );
      ...
      if (nonReq && !wrote) {
        log(
          "info",
          "blaze",
          `CTRL observe: no OUT written (session left open) in#=${appInCount} out#=${appOutCount} ctrl#=${ctrlObserveCount}`,
        );
      }
  ...
  const onClose = (why: string) => {
    ...
    log(
      "info",
      "blaze",
      `client disconnected (${mode}) ${remote} why=${why} aliveMs=${aliveMs} sinceCtrlMs=${sinceCtrlMs} appIn=${appInCount} appOut=${appOutCount} ctrlObserve=${ctrlObserveCount}`,
    );
  ...
  if (isProtoSsl) {
    ...
    (socket as ProtoSslSocket).socket.on("close", () => onClose("close"));
  } else {
    ...
    (socket as net.Socket).on("close", () => onClose("close"));
  }
}
