...
const issuedAuthCodes = new Map<
  string,
  { clientId: string; idToken: string; createdAt: number }
>();
...
function parseResponseTypes(responseType: string | undefined): Set<string> {
  return new Set(
    String(responseType ?? "")
      .split(/[\s+]+/)
      .map((t) => t.trim())
...
function buildOAuthRedirectTarget(
  redirectUri: string,
  opts: { code?: string; state?: string; idToken?: string },
): string {
  const redirectUrl = new URL(redirectUri);
  if (opts.code) redirectUrl.searchParams.set("code", opts.code);
  if (opts.state) redirectUrl.searchParams.set("state", opts.state);
  if (opts.idToken) {
...
function sendOAuthRedirect(res: express.Response, target: string): void {
  ...
  res
    ...
    .send(
      `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Origin redirect</title>` +
...
function issueTokenBundle(clientId: string, locale?: string, nonce?: string) {
...
function buildPidIdentityXml(): string {
...
function wantsXml(req: express.Request): boolean {
  ...
  if (accept.includes("json")) return false;
 ...
 */
export function createNucleusApp(): express.Express {
  const app = express();
  ...
  app.use((req, _res, next) => {
    ...
    log(
      "info",
      "nucleus",
      `★★ ${req.method} host=${req.headers.host ?? ""} path=${req.path}${q}${bodyHint}`,
    );
    ...
    if (isOriginish) {
      log(
        "info",
        "nucleus",
        `ORIGIN_REQUEST method=${req.method} path=${req.path}${q} accept=${req.headers.accept ?? "-"} auth=${req.headers.authorization ? "yes" : "no"} body=${bodyJson || "-"}`,
      );
    }
    next();
  });
  ...
  app.get("/connect/logout", (req, res) => {
    ...
    log(
      "info",
      "nucleus",
      `ORIGIN_LOGOUT_HIT client_id=${clientId} redirect_uri=${redirectUri} access_token=${queryStr(req.query.access_token) ? "yes" : "no"}`,
    );
    try {
      ...
      log(
        "warn",
        "nucleus",
        `ORIGIN_LOGOUT_REDIRECT_FAILED redirect_uri=${redirectUri} error=${(err as Error).message}`,
      );
  ...
  app.post("/connect/token", (req, res) => {
    const clientId =
      queryStr(req.body?.client_id) ?? queryStr(req.query.client_id) ?? "ORIGIN_PC";
    ...
    if (code) issuedAuthCodes.delete(code);
    log(
      "info",
      "nucleus",
      `ORIGIN_TOKEN_HIT grant_type=${grantType} client_id=${clientId} code=${code ?? "?"} remembered=${Boolean(remembered)}`,
    );
    res.json(bundle);
  });
  ...
  app.get("/connect/auth", (req, res) => {
    ...
    if (issuedAuthCodes.size > 64) {
      ...
      if (oldest) issuedAuthCodes.delete(oldest);
    }

    log(
      "info",
      "nucleus",
      `CONNECT_AUTH_HIT client_id=${clientId} response_type=${responseType ?? "?"} redirect_uri=${redirectUri ?? "?"} state=${state ?? "?"} locale=${locale}`,
    );
    ...
    if (wantsCode && typeof redirectUri === "string" && redirectUri.length > 0) {
      try {
        ...
        log(
          "info",
          "nucleus",
          `ORIGIN_AUTH_REDIRECT code=${code} state=${state ?? ""} redirect_uri=${redirectUri} has_id_token=${wantsIdToken}`,
        );
        ...
        log(
          "warn",
          "nucleus",
          `CONNECT_AUTH_REDIRECT_FAILED redirect_uri=${redirectUri} error=${(err as Error).message}`,
        );
      }
    }
    ...
    res
      ...
      .set("Content-Type", "text/html; charset=utf-8")
      .send(
        `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>EA Auth Stub</title>` +
  ...
  app.get(["/proxy/identity/pids/me", "/identity/pids/me", "/connect/userinfo"], (req, res) => {
    ...
    log(
      "info",
      "nucleus",
      `ORIGIN_PID_ME_HIT host=${req.headers.host ?? ""} accept=${req.headers.accept ?? "-"} xml=${asXml}`,
    );
    if (asXml) {
  ...
  app.get(
    ...
    (req, res) => {
      ...
      log(
        "info",
        "nucleus",
        `ORIGIN_PERSONAS_HIT path=${req.path} pid=${req.params.pid ?? "me"} ns=${ns} accept=${req.headers.accept ?? "-"}`,
      );
      const persona = {
  ...
  app.get(
    ["/proxy/identity/pids/:pid/profileinfo", "/identity/pids/:pid/profileinfo"],
    (req, res) => {
      ...
      if (wantsXml(req)) {
        return res
          ...
          .type("application/xml")
          .send(
            `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><pidProfile>` +
  ...
  app.get(
    ...
    (req, res) => {
      log(
        "info",
        "nucleus",
        `ORIGIN_SUBSCRIPTION_HIT pid=${req.params.pid} group=${req.params.group ?? "-"}`,
      );
  ...
  app.get(
    ...
    (req, res) => {
      log(
        "info",
        "nucleus",
        `ORIGIN_COMMERCE_HIT path=${req.path} pid=${req.params.pid} q=${JSON.stringify(req.query)}`,
      );
  ...
  app.get(["/", "/status", "/ping"], (req, res) => {
    if (String(req.headers.accept ?? "").includes("html") || req.path === "/") {
      return res
        ...
        .type("html")
        .send(
          `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Origin Local</title></head>` +
  ...
  const legalPages: Array<{ path: string; field: string; title: string }> = [
  ...
  for (const page of legalPages) {
    app.all(page.path, (req, res) => {
      log(
        "info",
        "nucleus",
        `LEGAL_PAGE_HIT field=${page.field} path=${page.path} host=${req.headers.host ?? ""} ua=${req.headers["user-agent"] ?? ""}`,
      );
      ...
      res
        ...
        .set("Content-Type", "text/html; charset=utf-8")
        .send(
          `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>${page.title}</title>` +
  ...
  app.all("/legal", (req, res) => {
    log(
      "info",
      "nucleus",
      `LEGAL_PAGE_HIT field=UNKNOWN path=/legal host=${req.headers.host ?? ""} ua=${req.headers["user-agent"] ?? ""}`,
    );
    ...
    res
      ...
      .set("Content-Type", "text/html; charset=utf-8")
      .send(
        `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Legal root</title>` +
          `<style>body{margin:0;background:#111;color:#fff;font-family:Segoe UI,sans-serif;` +
          `display:flex;min-height:100vh;align-items:center;justify-content:center;text-align:center}` +
          `h1{font-size:1.6rem;margin:0 0 .5rem}p{opacity:.85;margin:0}</style></head>` +
  ...
  app.all("/legal/*splat", (req, res) => {
    log(
      "info",
      "nucleus",
      `LEGAL_PAGE_HIT field=UNKNOWN path=${req.path} host=${req.headers.host ?? ""} ua=${req.headers["user-agent"] ?? ""}`,
    );
    log("info", "nucleus", `LEGAL_PAGE_HEADERS field=UNKNOWN ${JSON.stringify(req.headers)}`);
    res
      .status(200)
      .set("Content-Type", "text/html; charset=utf-8")
      .send(
        `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Legal fallback</title>` +
          `<style>body{margin:0;background:#111;color:#fff;font-family:Segoe UI,sans-serif;` +
          `display:flex;min-height:100vh;align-items:center;justify-content:center;text-align:center}` +
          `h1{font-size:1.6rem;margin:0 0 .5rem}p{opacity:.85;margin:0}</style></head>` +
...
export function startNucleusHttp(app: express.Express): void {
...
export function startNucleusHttps(app: express.Express, tls: { key: Buffer; cert: Buffer }): void {
  ...
  httpsServer.on("connection", (rawSocket) => {
    ...
    log(
      "info",
      "nucleus",
      `NUCLEUS_TLS_CONNECT peer=${socket.remoteAddress ?? "?"}:${socket.remotePort ?? "?"}`,
    );
  });
  httpsServer.on("secureConnection", (socket) => {
    log(
      "info",
      "nucleus",
      `NUCLEUS_TLS_CONNECT secure peer=${socket.remoteAddress ?? "?"}:${socket.remotePort ?? "?"} ` +
        `proto=${socket.getProtocol() ?? "?"} cipher=${socket.getCipher()?.name ?? "?"}`,
    );
  });
  httpsServer.on("tlsClientError", (err, rawSocket) => {
    const socket = rawSocket as Socket;
    const tlsSocket = rawSocket as Socket & {
      servername?: string;
    ...
    log(
      "warn",
      "nucleus",
      `NUCLEUS_TLS_ERROR peer=${socket.remoteAddress ?? "?"}:${socket.remotePort ?? "?"} ` +
