...
import { loadLsxSession, type LsxSession } from "./session.js";
import {
  buildResponse,
...
function tag(msg: string): void {
  log("info", "lsx", msg);
}

function sendRaw(socket: net.Socket, bytes: Buffer | string): void {
  const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes, "utf8");
  const out = Buffer.concat([buf, Buffer.from([0x00])]);
  socket.write(out);
}

function sendPlainXml(socket: net.Socket, xml: string): void {
  sendRaw(socket, xml);
}
...
export async function startOriginLsxServer(
  opts: LsxServerOptions = {},
): Promise<net.Server> {
  assertLsxCryptoVectors();
  ...
  if (await portInUse(host, port)) {
    const msg =
      `LSX_PORT_BUSY ${host}:${port} — Origin/EA Desktop tient déjà le port. ` +
      `Ferme Origin.exe puis relance (PID actuel via: netstat -ano | findstr :${port}).`;
    tag(msg);
    if (failIfBusy) throw new Error(msg);
  }

  const server = net.createServer((socket) => {
    handleClient(socket, session).catch((e) => {
      tag(`client error ${(e as Error).message}`);
      try {
        socket.destroy();
      } catch (_) {}
    });
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      tag(
        `LSX_LISTENING ${host}:${port} uid=${session.uid} persona=${session.personaName} auth=${session.authCode}`,
      );
      resolve();
    });
  });
...
async function handleClient(
  ...
  const flushMessages = () => {
    while (true) {
      ...
      if (msg.length) onMessage(msg);
    }
  };

  const maybeSendOnlineEvents = () => {
    if (eventsSent || !authed) return;
    eventsSent = true;
  ...
  const onMessage = (raw: Buffer) => {
    ...
    if (!authed) {
      ...
      if (cr) {
        ...
        if (cr.response !== expectedResponse) {
          tag(
            `LSX_CHALLENGE_RESPONSE_DIFF client=${cr.response.slice(0, 16)}… expected=${expectedResponse.slice(0, 16)}…`,
          );
        }
        ...
        sendPlainXml(
          socket,
          responseChallengeAccepted(cr.id, acceptedResponse),
        );
        authed = true;
        tag(
          `LSX_CHALLENGE_ACCEPTED id=${cr.id} clientKey=${cr.key} response=${acceptedResponse.slice(0, 16)}…`,
        );
    ...
    try {
      ...
      tag(`LSX_DECRYPT_FAIL ${(e as Error).message} raw=${asUtf.slice(0, 80)}`);
      return;
    }
    ...
    tag(
      `LSX_REQUEST type=${meta.type} id=${meta.id} recipient=${meta.recipient}` +
    ...
    if (!response) {
      ...
      const soft = responseXml(
        meta.id,
        meta.recipient || "EbisuSDK",
        `<ErrorSuccess Code="0" Description="Success"/>`,
      );
    ...
    tag(
      `LSX_RESPONSE type=${meta.type} id=${meta.id} sender=${response.sender}`,
    );
    if (process.env.LSX_TRACE_XML === "1") {
