...
export function wrapLsx(inner: string): string {
...
export function eventChallenge(
  ...
  return wrapLsx(
    `<Event sender="EALS"><Challenge key="${esc(key)}" version="${esc(version)}" build="${esc(build)}"/></Event>`,
  );
}

export function responseChallengeAccepted(id: string, responseHex: string): string {
  return wrapLsx(
    `<Response id="${esc(id)}" sender="EALS"><ChallengeAccepted response="${esc(responseHex)}"/></Response>`,
  );
}

export function responseXml(id: string, sender: string, body: string): string {
  return wrapLsx(
    `<Response id="${esc(id)}" sender="${esc(sender)}">${body}</Response>`,
  );
}

export function eventXml(sender: string, body: string): string {
...
function success(sender: string): LsxResponse {
  return {
    sender,
...
export function buildResponse(
  ...
  switch (type) {
    ...
    case "GetGameInfo": {
      ...
      if (infoId === "FREETRIAL") value = "false";
      else if (infoId === "UPTODATE") value = "true";
      else if (infoId === "INSTALLED_LANGUAGE") value = "fr_FR";
      return {
        sender: "EbisuSDK",
    ...
    case "GetSetting": {
      ...
      let value = "false";
      if (settingId === "ENVIRONMENT") value = "production";
      else if (settingId === "LANGUAGE") value = "fr_FR";
      else if (settingId === "IS_IGO_AVAILABLE") value = "true";
      else if (settingId === "IS_IGO_ENABLED") value = "true";
      return {
        sender: "EbisuSDK",
    ...
    case "GetProfile":
      return {
        sender: "EbisuSDK",
        body:
          `<GetProfileResponse UserIndex="0" UserId="${session.uid}" PersonaId="${session.personaId}" ` +
    ...
    case "GetAllGameInfo":
      return {
        sender: "EbisuSDK",
        body:
          `<GetAllGameInfoResponse UpToDate="true" Languages="fr_FR,en_US" FreeTrial="false" ` +
    ...
    case "GetPresence":
      return {
        sender: "XMPP",
        body:
          `<GetPresenceResponse UserId="${session.uid}" Presence="ONLINE" Title="${esc(session.displayName)}" ` +
    ...
    case "GetSettings":
      return {
        sender: "EbisuSDK",
        body:
          `<GetSettingsResponse Language="fr_FR" Environment="production" ` +
...
export function onlineStatusEvent(isOnline: boolean): string {
  return eventXml(
    "EbisuSDK",
    `<OnlineStatusEvent isOnline="${isOnline ? "1" : "0"}"/>`,
  );
}

export function currentUserPresenceEvent(session: LsxSession): string {
  return eventXml(
    "EbisuSDK",
    `<CurrentUserPresenceEvent UserId="${session.uid}" Presence="ONLINE" Title="${esc(session.displayName)}" ` +
      `TitleId="${esc(session.contentId)}" MultiplayerId="" RichPresence="" GamePresence="" ` +
      `SessionId="local" Group="" GroupId=""/>`,
  );
}

export function profileEvent(session: LsxSession): string {
  return eventXml(
    "EbisuSDK",
    `<ProfileEvent Changed="EAID" UserId="${session.uid}"/>`,
  );
}

/** Extract request id + first child element name from LSX Request XML. */
export function parseRequestMeta(xml: string): {
  id: string;
  type: string;
  recipient: string;
  attributes: Record<string, string>;
  ...
  const bodyM = xml.match(
    /<Request\b[^>]*>\s*<([A-Za-z0-9_]+)([^>]*)/i,
  );
...
export function loginEvent(): string {
  return eventXml(
    "EbisuSDK",
...
export function parseChallengeResponse(xml: string): {
  id: string;
  key: string;
  response: string;
} | null {
  if (!/ChallengeResponse/i.test(xml)) return null;
  const idM = xml.match(/<Request[^>]*\bid="([^"]*)"/i);
  const keyM = xml.match(/<ChallengeResponse[^>]*\bkey="([^"]*)"/i);
  const respM = xml.match(/<ChallengeResponse[^>]*\bresponse="([^"]*)"/i);
  if (!keyM || !respM) return null;
  return {
    id: idM?.[1] ?? "0",
    key: keyM[1],
    response: respM[1],
