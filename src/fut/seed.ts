...
export const SEED_PLAYERS: Array<{
  asset_id: number;
  name: string;
  rating: number;
  position: string;
...
export function seedPlayers(): void {
...
export function ensureAccountClub(personaId: number, nucleusId: number, displayName: string) {
  const database = getDb();
  let account = database.prepare(`SELECT * FROM accounts WHERE persona_id = ?`).get(personaId) as
  ...
  if (!account) {
    ...
    account = database.prepare(`SELECT * FROM accounts WHERE persona_id = ?`).get(personaId) as {
      id: number;
      display_name: string;
    };
  }

  let club = database.prepare(`SELECT * FROM clubs WHERE account_id = ?`).get(account.id) as
  ...
  if (!club) {
    database
      .prepare(
        `INSERT INTO clubs (account_id, club_name, club_abbr, coins) VALUES (?, ?, 'LOC', 7500)`,
      )
      .run(account.id, `${displayName} FC`);
    club = database.prepare(`SELECT * FROM clubs WHERE account_id = ?`).get(account.id) as Record<
      string,
      unknown
    >;
    ...
    const insertItem = database.prepare(
      `INSERT INTO items (club_id, asset_id, item_type, untradeable, discard_value) VALUES (?, ?, 'player', 1, 350)`,
    );
    ...
    for (const assetId of STARTER_ASSET_IDS) {
      ...
      const row = database
        ...
        .get(clubId, assetId) as { id: number };
      slotIds.push(row.id);
    }
    ...
    const squad = database.prepare(`SELECT id FROM squads WHERE club_id=?`).get(clubId) as { id: number };
    const insertSlot = database.prepare(
      `INSERT OR REPLACE INTO squad_players (squad_id, slot_index, item_id) VALUES (?, ?, ?)`,
    );
    slotIds.forEach((itemId, index) => insertSlot.run(squad.id, index, itemId));
    database.prepare(
      `INSERT OR REPLACE INTO fut_users (persona_id, active_squad_id, starter_pack_claimed, tutorial_complete)
       VALUES (?, ?, 1, 1)`,
    ).run(personaId, squad.id);
  ...
  database.prepare(
    `INSERT OR IGNORE INTO fut_users (persona_id, starter_pack_claimed, tutorial_complete) VALUES (?, 1, 1)`,
  ).run(personaId);
  return { account, club };
}
