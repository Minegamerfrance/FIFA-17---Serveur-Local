...
function personaFromReq(req: express.Request): number {
  const header = req.header("X-UT-SID") || req.header("Easw-Session-Data") || "";
  const q = req.query.personaId;
  if (typeof q === "string" && q) return Number.parseInt(q, 10);
  if (header.startsWith("persona:")) return Number.parseInt(header.slice(8), 10);
  return config.defaultPersonaId;
}

function createFutApp(): express.Express {
  initFutData();
  ...
  app.get(
    ["/ut/game/fifa17/phishing", "/ut/game/fifa17/phishing/question"],
    (_req, res) => {
  ...
  app.post("/ut/game/fifa17/user/action", (req, res) => {
    ...
    if (!result) return res.status(400).json({ error: "invalid_action" });
    res.json(result);
  });
  ...
  app.post("/ut/game/fifa17/purchased/packs", (req, res) => {
    ...
    const club =
      getClubByPersona(personaId) ??
      getOrCreateClub(personaId, config.defaultNucleusId, config.defaultPersonaName);
    const packType = (req.body?.packType as string) || "premium_gold";
  ...
  app.post("/ut/game/fifa17/purchased/packs/:packId/open", (req, res) => {
    ...
    if (!result) return res.status(404).json({ error: "pack_not_found" });
    res.json(result);
  });
...
export function startFutApi(tls: { key: Buffer; cert: Buffer }): express.Express {
