...
export function getDb(): DatabaseSync {
  ...
  if (fs.existsSync(config.databasePath)) fs.copyFileSync(config.databasePath, `${config.databasePath}.backup`);
...
function migrate(database: DatabaseSync): void {
  ...
  if (!itemNames.has("pile")) database.exec(`ALTER TABLE items ADD COLUMN pile TEXT NOT NULL DEFAULT 'club'`);
  if (!itemNames.has("resource_id")) database.exec(`ALTER TABLE items ADD COLUMN resource_id INTEGER`);

  const addColumn = (table: string, column: string, sql: string) => {
    const columns = database.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>;
    if (!columns.some((entry) => entry.name === column)) database.exec(`ALTER TABLE ${table} ADD COLUMN ${sql}`);
  };
