...
function pruneOldServerLogs(): void {
  if (RETENTION_DAYS <= 0) return;
  const cutoff = Date.now() - RETENTION_DAYS * 24 * 60 * 60 * 1000;
  try {
    for (const name of fs.readdirSync(config.logDir)) {
      if (!/^server-\d{4}-\d{2}-\d{2}/.test(name)) continue;
...
function createRootLogger(): pino.Logger {
  ...
  try {
    fs.writeFileSync(
      path.join(config.logDir, "server-latest.txt"),
      sessionLogPath + "\n",
      "utf8",
    );
  } catch {
...
export function announceSessionLog(): void {
...
export function dumpPacket(label: string, buf: Buffer): void {
  ensureLogDir();
