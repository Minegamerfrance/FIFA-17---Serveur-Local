...
export function listenHttpAndHttps(
  ...
  httpsServer.on("secureConnection", (socket) => {
    log(
      "info",
      label,
      `TLS OK :${port} ${socket.remoteAddress}:${socket.remotePort} ${socket.getProtocol()} ${socket.getCipher()?.name ?? "?"}`,
    );
  });
  ...
  const server = net.createServer((socket) => {
    ...
    socket.once("data", (first) => {
      ...
      log(
        "info",
        label,
        `:${port} from ${remote} proto=${isTls ? "TLS" : "HTTP/raw"} len=${first.length} hex=${head.toString("hex")} ascii=${ascii}`,
      );
      dumpPacket(`${label}:first:${port}`, first.subarray(0, Math.min(512, first.length)));

      socket.pause();
      socket.unshift(first);
      if (isTls) httpsServer.emit("connection", socket);
