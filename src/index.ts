...
function startPasProtocolProbe(): void {
  const port = 8094;
  const pasResponseFor = (method: string, url: string): unknown => {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      log(
        "info",
        "pas",
        `PAS_HTTP_REQUEST peer=${peer} method=${method} url=${url} contentType=${req.headers["content-type"] ?? ""} len=${body.length} body=${JSON.stringify(body.toString("utf8"))}`,
      );
      ...
      log(
        "info",
        "pas",
        `PAS_HTTP_RESPONSE peer=${peer} status=200 body=${JSON.stringify(responseText)}`,
      );
    });
  });
  server.on("error", (err) => {
...
async function main(): Promise<void> {
  ...
  const authNotifyDelayMs = Math.max(
    0,
  ...
  log(
    "info",
    "main",
    `AUTH_REPLY_PROFILE=${authProfile} notify=${authNotify ? "on" : "off"} notifyDelayMs=${authNotifyDelayMs} (plst|plst-reference|plst-legal-local|plst-legal-ea|full|persona|empty|none)`,
  );
  ...
  if (legalBase || /legal/i.test(authProfile)) {
    log(
      "info",
      "main",
      `AUTH_LEGAL_BASE_URL=${legalBase || "(profile default)"} legal paths=/legal/{privacy,terms-ui,terms,ldht,thst}`,
    );
  }
  log(
    "info",
    "main",
    `host=${config.host} redirector=${config.redirectorPort}+TLS:${config.engagementPort} blaze=${config.blazePort} nucleus=${config.nucleusPort}/443 fut=${config.futPort}/${config.futPortAlt}`,
  );
  ...
  try {
    ...
    log("warn", "main", `LSX optional start failed: ${(e as Error).message}`);
  }
