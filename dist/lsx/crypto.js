...
export class LsxCrypto {
    ...
    encrypt(plainText) {
        if (!plainText)
            throw new Error("LSX encrypt: empty input");
    ...
    decrypt(cipherText) {
        if (!cipherText.length)
            throw new Error("LSX decrypt: empty input");
     ...
     */
    prepareChallengeResponse(challengeKey) {
    ...
    applySessionFromResponseHex(responseHex) {
...
export function assertLsxCryptoVectors() {
    ...
    if (round !== "hello world")
        throw new Error("LSX crypto roundtrip failed");
}
