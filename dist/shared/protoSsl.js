...
export function loadBlazeProtoSslMaterial() {
    ...
    try {
        ...
        log("info", "protoss", `cert ${path.basename(certPath)} CN=${x.subject.split("\n").find((l) => l.startsWith("CN=")) ?? "?"} len=${certDer.length}`);
    }
    catch (_) {
...
function generateKeyBlock(out, key, rand1, rand2) {
    let i = 1;
    let saltByte = 0x41;
...
function createKeys(pmKey, clientRandom, serverRandom) {
...
function computeSha1Mac(writeSecret, ty, message, seq) {
...
function computeFinished(masterSecret, isClient, transcript) {
...
export async function acceptProtoSsl(socket, material, firstChunk) {
    ...
    const readMore = () => new Promise((resolve, reject) => {
    ...
    const readRecord = async () => {
        while (buffer.length < 5)
            buffer = Buffer.concat([buffer, await readMore()]);
        const type = buffer[0];
        const len = buffer.readUInt16BE(3);
        while (buffer.length < 5 + len)
            buffer = Buffer.concat([buffer, await readMore()]);
        const payload = Buffer.from(buffer.subarray(5, 5 + len));
        buffer = buffer.subarray(5 + len);
        return { type, payload };
    };
    const readRecordAt = async (stage) => {
        hsStage = stage;
        try {
            return await readRecord();
        }
        catch (err) {
            hsFail(`read record failed: ${err.message}`);
            throw err;
        }
    };
    const writeRaw = (data) => {
    ...
    const writeHandshakeRecord = (label, payload) => {
        try {
            writeRaw(record(MT.Handshake, payload));
            log("info", "protoss", `sent ${label} recLen=${payload.length} peer=${peer}`);
        }
        catch (err) {
            hsFail(`write ${label} failed: ${err.message}`);
        }
    };
    const transcriptParts = [];
    let peerFinishedAt = -1;
    const appendTranscript = (handshakeMsg) => {
        transcriptParts.push(Buffer.from(handshakeMsg));
    };
    const fullTranscript = () => Buffer.concat(transcriptParts);
    const peerTranscript = () => peerFinishedAt < 0 ? fullTranscript() : fullTranscript().subarray(0, peerFinishedAt);
    const first = await readRecordAt("read-client-hello");
    if (first.type !== MT.Handshake)
        throw new Error(`expected Handshake, got ${first.type}`);
    ...
    if (hsType !== HT.ClientHello)
        hsFail(`expected ClientHello, got ${hsType}`);
    ...
    const certEntries = Buffer.concat(certs.map((der) => Buffer.concat([u24(der.length), der])));
    ...
    log("info", "protoss", `HEX ClientHello bodyVer=0x${clientVersion.toString(16)} random=${clientRandom.toString("hex")} raw=${hsRaw.toString("hex")}`);
    log("info", "protoss", `HEX ServerHello rec=${shRec.toString("hex")} body=${serverHelloBody.toString("hex")} (ver=0x0300 cipher=0x0005 sessIdLen=0 comp=0)`);
    log("info", "protoss", `HEX Certificate recLen=${certRec.length} leafDerLen=${material.certDer.length} head=${certRec.subarray(0, Math.min(48, certRec.length)).toString("hex")}…`);
    ...
    log("info", "protoss", `sent ServerHello+Cert+Done ver=0x0300 (ssl3) certLen=${material.certDer.length} clientHelloVer=0x${clientVersion.toString(16)} records=3 — waiting CKE`);
    ...
    for (let skip = 0; skip < 4; skip++) {
        ...
        if (ckeRec.type === MT.Alert) {
            ...
            throw new Error(`client SSL alert level=${level} desc=${desc} (${names[desc] ?? "unknown"}) hex=${ckeRec.payload.toString("hex")}`);
        }
        ...
        if (hsType === HT.ClientKeyExchange)
            break;
    ...
    if (ckeType !== HT.ClientKeyExchange)
        hsFail(`expected ClientKeyExchange, got ${ckeType}`);
    ...
    try {
        pmSecret = crypto.privateDecrypt({ key: material.privateKey, padding: crypto.constants.RSA_PKCS1_PADDING }, encryptedPm);
    }
    catch (err) {
        hsFail(`premaster decrypt failed: ${err.message}`);
    }
    if (pmSecret.length !== 48) {
        if (pmSecret.length > 48)
            pmSecret = pmSecret.subarray(pmSecret.length - 48);
        else
            hsFail(`bad premaster length ${pmSecret.length}`);
    }
    const keys = createKeys(pmSecret, clientRandom, serverRandom);
    let clientSeq = 0n;
    let serverSeq = 0n;
    const ccs = await readRecordAt("wait-client-ccs");
    if (ccs.type !== MT.ChangeCipherSpec)
        hsFail(`expected CCS, got ${ccs.type}`);
    const finRec = await readRecordAt("wait-client-finished");
    if (finRec.type !== MT.Handshake)
        hsFail(`expected Finished Handshake, got ${finRec.type}`);
    ...
    if (!mac.equals(expectMac))
        hsFail("client Finished MAC mismatch");
    ...
    if (finType !== HT.Finished)
        hsFail(`expected Finished, got ${finType}`);
    peerFinishedAt = fullTranscript().length;
    const expectedFin = computeFinished(keys.masterKey, true, peerTranscript());
    if (!finBody.equals(expectedFin))
        hsFail("client Finished hash mismatch");
    ...
    const handleAppRecords = () => {
        while (appBuffer.length >= 5) {
            ...
            if (type === MT.Alert) {
                ...
                log("warn", "protoss", `alert level=${level} desc=${desc} hex=${payload.toString("hex")}`);
                socket.destroy();
                return;
            }
            if (type !== MT.ApplicationData)
                continue;
            keys.clientRc4.process(payload);
...
//# sourceMappingURL=protoSsl.js.map