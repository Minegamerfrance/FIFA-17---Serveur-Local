import { TdfType, TdfReader, fieldToObject } from "../shared/tdf.js";
 ...
 */
export function preAuthHeaderStyle(_req) {
    return "fire2";
}
const REDIRECTOR_SERVICE = () => process.env.REDIRECTOR_SERVICE_NAME?.trim() || "fifa-2017-pc";
...
function advertisedComponentIds() {
...
function observeRequest(req) {
    ...
    try {
        const root = fieldToObject(new TdfReader(req.payload).readStructFields());
        const cinf = (root.CINF ?? {});
        const cdat = (root.CDAT ?? {});
        return {
...
function writeMinimalQos(w, qosHost) {
 ...
 */
export function writePreAuthReply(req, w) {
    ...
    log("info", "blaze-trace", `PREAUTH style=${style} INST=${obs.svcn} SVER_BSDK=${obs.bsdk} CLNT=${obs.clnt} CSKU=${obs.csku} CVER=${obs.cver} ASRC=${JSON.stringify(authSource)} CIDS=${JSON.stringify(componentIds)} RSRC=${JSON.stringify(registrationSourceValue)}`);
    ...
    w.writeList("CIDS", TdfType.Integer, componentIds.map((componentId) => (item) => {
