...
function authSpamValue(defaultValue) {
...
function authLegalUrlsEnabled() {
    return authLegalProfileKind() !== null;
}
function authLegalBaseUrl() {
    const fromEnv = (process.env.AUTH_LEGAL_BASE_URL ?? "").trim();
    if (fromEnv)
        return fromEnv.replace(/\/$/, "");
    if (authLegalProfileKind() === "ea")
        return "https://accounts.ea.com";
 ...
 */
export function writeLoginResponse(w, session) {
...
export function writeFullLoginResponse(w, session) {
    const now = Math.floor(Date.now() / 1000) >>> 0;
    const accountReady = authAccountReady();
    const spam = authSpamValue(accountReady ? 0 : 1);
 ...
 */
export function writeSessionInfoReply(w, session) {
    const now = Math.floor(Date.now() / 1000) >>> 0;
...
export function writeOriginTokenLoginReply(w, session) {
    const profile = (process.env.AUTH_REPLY_PROFILE ?? "plst").trim().toLowerCase();
    if (profile === "full") {
...
function buildNotifyUserAdded(session, headerStyle) {
...
function buildNotifyUserUpdated(session, headerStyle) {
    const w = new TdfWriter();
 ...
 */
function buildNotifyUserAuthenticated(session, headerStyle) {
    const now = Math.floor(Date.now() / 1000) >>> 0;
    const w = new TdfWriter();
...
function buildNotifyExtendedDataUpdate(session, headerStyle) {
    const w = new TdfWriter();
    w.writeStruct("DATA", (data) => {
 ...
 */
function buildNotifyServerCensusData(headerStyle) {
    const w = new TdfWriter();
...
function readLocalizationRequest(payload) {
    try {
        ...
        const lang = langField?.value.type === TdfType.Integer
            ? Number(langField.value.value)
            : 0x66724652; // "frFR" in Blaze locale form
        const ids = lsid?.value.type === TdfType.List
            ? lsid.value.value
        ...
        log("warn", "blaze", `util_4 localization request decode failed: ${e instanceof Error ? e.message : String(e)}`);
        return { ids: [], lang: 0x66724652 };
    }
}
function readIntegerField(payload, tag, fallback) {
    try {
        const fields = new TdfReader(payload).readStructFields();
...
function readStringField(payload, tag, fallback) {
    try {
        const fields = new TdfReader(payload).readStructFields();
        const field = fields.find((item) => item.tag === tag);
...
export function handleBlazeRequestLegacy(req, session) {
    const replies = [];
    ...
    if (req.component === Component.Authentication) {
        ...
        if (req.command === 0x0a) {
            ...
            if (profile === "none" || profile === "silent") {
                log("info", "blaze", "AUTH_REPLY_PROFILE none — NO reply to Auth/10 (layer test; expect timeout shift if Blaze reply mattered)");
                return replies;
            }
            const legalKind = authLegalProfileKind();
            const legal = legalKind ? authLegalUrls() : null;
            const profileLog = legalKind
            ...
            log("info", "blaze", `AUTH_REPLY_PROFILE=${profile} ${profileLog} accountReady=${accountReady ? "on" : "off"} AGUP=${accountReady ? 1 : 0} SPAM=${spam} notify=${notify ? "UserAuthenticated+UserAdded+ExtendedData" : "off"}`);
            if (legal) {
                log("info", "blaze", `AUTH_LEGAL_URLS PRIV=${legal.PRIV} TSUI=${legal.TSUI} TURI=${legal.TURI} LDHT=${legal.LDHT} THST=${legal.THST}`);
            }
            replies.push(tdfReply(req, (w) => {
        ...
        if (req.command === 0x20 || req.command === 0x1d) {
            ...
            try {
                ...
                if (groups?.value.type === TdfType.List) {
                    const decoded = groups.value.value
                        ...
                        .filter((value) => value.length > 0);
                    if (decoded.length > 0)
                        requestedGroups = decoded;
                }
            }
            catch (_) { }
            replies.push(tdfReply(req, (w) => {
                w.writeList("NLST", TdfType.Struct, requestedGroups.map((groupName, index) => (entitlement) => {
            ...
            log("info", "blaze", `AUTH_ENTITLEMENTS groups=${requestedGroups.join(",")} entries=${requestedGroups.length}`);
            return replies;
        }
        replies.push(tdfReply(req, (w) => {
            w.writeInteger("BUID", session.blazeId);
    ...
    if (req.component === Component.Util) {
        ...
        if (req.command === 4 /* localizeStrings */) {
            ...
            const ids = localizationRequest.ids.length > 0 ? localizationRequest.ids : Object.keys(LOCALIZED_STRINGS);
            const seen = new Set();
            const entries = ids
                .filter((id) => {
                if (seen.has(id))
            ...
            log("info", "blaze", `BLAZE_LOCALIZE_MODE ${localizeMode} lang=${localizationRequest.lang} ids=${entries.length} first=${entries[0]?.key ?? "none"}`);
            replies.push(tdfReply(req, (w) => {
                if (localizeMode === "smap") {
                    ...
                    w.writeStringList("STRS", entries.map((entry) => entry.value));
                }
                else if (localizeMode === "usertext") {
                    w.writeList("STRS", TdfType.Struct, entries.map((entry) => (userText) => {
                    ...
                    w.writeMap("STRS", TdfType.Integer, TdfType.Struct, [
                        {
                            ...
                            writeValue: (userStringList) => {
                                userStringList.writeList("STRS", TdfType.Struct, entries.map((entry) => (userText) => {
                                    userText.writeString("LSID", entry.key);
                                    userText.writeString("TEXT", entry.value);
                                    userText.endStruct();
                                }));
        ...
        if (req.command === 0x16 /* updateNetworkInfo / client network state */) {
            ...
            log("info", "blaze", `UTIL_22 updateNetworkInfo echo UBFL=${ubfl} UDEV=${JSON.stringify(udev)} UFLG=${uflg} ULRC=${ulrc} UNAT=${unat} USTA=${usta}`);
            replies.push(tdfReply(req, (w) => {
        ...
        if (req.command === 7 /* preAuth */) {
            ...
            }, req.component, req.command, preAuthHeaderStyle(req)));
            return replies;
        }
    ...
    if (req.component === Component.Stats && req.command === 15) {
        replies.push(tdfReply(req, (w) => w.writeMap("KSIT", TdfType.String, TdfType.Struct, [])));
        return replies;
    }
    ...
    if (req.component === 0x081c && req.command === 3) {
        replies.push(tdfReply(req, (w) => w.writeString("URL", "http://127.0.0.1:8000/sponsored-events")));
        return replies;
    }
    // Association lists / messaging / stats — acknowledge
    if (req.component === Component.AssociationLists ||
...
function handleGameManager(req, session) {
    const replies = [];
    ...
    if (match) {
        ...
        const notify = (target, opponent) => buildBlazePacket({
...
export function buildMatchResultNotification(gameId, winnerBlazeId, scoreHome, scoreAway) {
