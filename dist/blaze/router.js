import { buildBlazePacket, MsgType, Component } from "../shared/blazePacket.js";
import { log } from "../shared/logger.js";
import { handleBlazeRequestLegacy } from "./handlers.js";
import { utilCommandName } from "./trace.js";
const handlers = new Map();
export function registerHandler(component, command, handler) {
 ...
 */
export function isBlazeNonRequestFrame(req) {
    if (req.msgType !== MsgType.Message)
        return true;
    if (req.component === 0 &&
        req.command === 0 &&
...
export function routeBlazeRequest(req, session, write) {
    if (isBlazeNonRequestFrame(req)) {
        log("info", "blaze", `CTRL/non-request OBSERVE (no reply) component=${req.component} command=${req.command} msgType=${req.msgType} options=${req.options} msgNum=${req.msgNum} error=${req.error} payloadLen=${req.payload.length}`);
        return;
    }
    const utilLabel = req.component === Component.Util ? utilCommandName(req.command) : `${req.component}/${req.command}`;
