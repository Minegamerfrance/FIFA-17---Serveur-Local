...
function buildAddressBlock(host, port, addrType) {
    const style = (process.env.REDIRECTOR_ADDR_STYLE ?? "member").toLowerCase();
    const body = `      <hostname>${host}</hostname>\n` + `      <port>${port}</port>\n`;
    if (style === "valu" || style === "val") {
        const ipDotted = process.env.REDIRECTOR_IP?.trim() ||
        ...
        if (ipFormat !== "omit" && ipFormat !== "none") {
            ...
            const ipXml = ipFormat === "dotted" || ipUint === null ? ipDotted : String(ipUint);
            ipLine = `      <ip>${ipXml}</ip>\n`;
        }
        return (`  <address>\n` +
            `    <val>${addrType}</val>\n` +
...
function buildServerInstanceXml() {
    ...
    const addrType = (process.env.REDIRECTOR_ADDR_TYPE ?? "INTERNAL_IPPORT").toUpperCase() ===
        "EXTERNAL_IPPORT"
        ? "EXTERNAL_IPPORT"
        : "INTERNAL_IPPORT";
...
function buildHttpResponse(body, contentType) {
...
export function handleRedirectorConnection(socket, mode, initial) {
    ...
    const consume = () => {
        ...
        if (testMode === "nobody" || testMode === "empty" || testMode === "headers") {
            ...
            log("warn", "redirector", `DIFFERENTIAL TEST REDIRECTOR_TEST=${testMode} — HTTP 200 Content-Length:0 (no XML body) bytes=${res.length}`);
        }
        else {
    ...
    if (isProtoSsl) {
        ...
        socket.socket.on("error", (err) => log("warn", "redirector", `socket error: ${err.message}`));
        socket.socket.on("close", () => log("info", "redirector", `client disconnected (${mode}) ${remote}`));
    }
    else {
        ...
        socket.on("error", (err) => log("warn", "redirector", `socket error: ${err.message}`));
        socket.on("close", () => log("info", "redirector", `client disconnected (${mode}) ${remote}`));
    }
}
function closeSocket(isProtoSsl, socket) {
    if (isProtoSsl)
        socket.end();
    else
        socket.end();
