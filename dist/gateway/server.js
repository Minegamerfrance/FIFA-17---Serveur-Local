...
import { acceptProtoSsl, loadBlazeProtoSslMaterial, } from "../shared/protoSsl.js";
import { handleRedirectorConnection } from "../redirector/server.js";
import { handleBlazeConnection } from "../blaze/server.js";
const FIFA_CIPHERS = "RC4-SHA:RC4-MD5:AES128-SHA:AES256-SHA:AES128-SHA256:AES256-SHA256:AES128-GCM-SHA256:AES256-GCM-SHA384:@SECLEVEL=0";
function isClientAbortDuringHandshake(err) {
...
export function startGateway() {
    ...
    redirectorPlain.on("error", (err) => {
        log("warn", "gateway", `Redirector (Plain) ${config.host}:${config.redirectorPort} unavailable (${err.message}) — continuing with ProtoSSL redirector`);
    });
    ...
    if (useNodeTls) {
        ...
        tlsServer.on("error", (err) => {
            log("error", "gateway", `Redirector (Node TLS) ${config.host}:${sslPort} FATAL ${err.message} — kill stale node or re-run start:current`);
            process.exit(1);
        });
        ...
        protoServer.on("error", (err) => {
            log("error", "gateway", `Redirector (ProtoSSL) ${config.host}:${sslPort} FATAL ${err.message} — kill stale node or re-run start:current`);
            process.exit(1);
        });
    ...
    const blazePorts = Array.from(new Set([
        config.blazePort,
    ...
    for (const port of blazePorts) {
        ...
        blazeServer.on("error", (err) => {
            log("error", "gateway", `Blaze Core ${config.host}:${port} FATAL ${err.message} — kill stale node or re-run start:current`);
            process.exit(1);
        });
        blazeServer.listen(port, config.host, () => {
            log("info", "gateway", `Blaze Core listening on ${config.host}:${port} (plain+ProtoSSL)`);
        });
    }
}
async function handleBlazeInbound(socket, material) {
    ...
    try {
        ...
        log("info", "gateway", `Blaze first-byte from ${remote} tlsGuess=${isTls} httpGuess=${isHttp} len=${first.length} first=${first.subarray(0, 8).toString("hex")}`);
        if (isHttp) {
            log("warn", "gateway", `Blaze port got HTTP (likely FORCE_BLAZE redirected FUT) — not Blaze protocol; closing`);
            socket.destroy();
            return;
        }
        if (isTls) {
...
async function handleRedirectorPlain(socket, material) {
    try {
        ...
        log("warn", "gateway", `Redirector plain setup failed: ${err.message}`);
        socket.destroy();
    }
}
async function handleRedirectorProtoSsl(socket, material, sslPort) {
    try {
        const first = await readFirstChunk(socket);
        const ssl = await acceptProtoSsl(socket, material, first);
        ...
        log("warn", "gateway", `ProtoSSL :${sslPort} failed: ${err.message}`);
        socket.destroy();
    }
}
function readFirstChunk(socket, timeoutMs = 15000) {
