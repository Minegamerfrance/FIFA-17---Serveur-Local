...
function onTcpConnectAttempt(api, fd, sa, ctx) {
  ...
  if (BLAZE_PORTS.indexOf(port) >= 0) {
    ...
    // Do NOT tryAdopt/softHost/SetState on Blaze — sa-256 often resolves the wrong
    // ProtoSSL object (iState stuck at 0). Writing host/flag there corrupts heap and
    // crashes FIFA after PreAuth. Alert-42 block alone is enough (proven v112/v113).
    if (BLAZE_TLS_TRACE) {
  ...
  if (afterRedir) {
    proofPostRedirConnects++;
    // NO backtrace — Thread.backtrace on QoS/ephemeral connect after PreAuth crashes FIFA.
    proof(
      "★★ CONNECT_UNEXPECTED " + api + " :" + port + " ip=" + ip + " fd=" + fd + " (no-bt)",
    );
  }
}
...
function tryBlockBadCertAlert(buf, len, argsLenIndex, tag) {
  ...
  try {
    ...
    const desc = buf.add(6).readU8();
    if (desc !== 42 && desc !== 43 && desc !== 46) return false;
