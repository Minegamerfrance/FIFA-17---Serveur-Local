/**
 * FIFA 17 ProtoSSL bypass v55
 *
 * Gemini + evidence:
 * - FAIL_9 is AFTER successful _ParseCertificate (KeyMod @ pSecure+0x24b0).
 * - Causes: hostname / CA trust / date — NOT parse.
 * - DO: make verify path return 0 so DirtySDK reaches HelloDone → natural CKE.
 * - DO NOT: force iState, inject KeyMod, flush SendData, mov 0x1009→21.
 *
 * Why v53 xor@612d548 failed:
 * - That call→0x6129eb0 is only ONE test/je gate (likely hostname or a helper).
 * - DirtySDK order: WildcardMatch THEN _VerifyCertificate — a second gate remains.
 * - v54 je-belt never applied (PATTERN e8… broken after xor); FAIL_9 jcc scan = 0 hits.
 *
 * v55:
 * - softHost CN early + sticky (re-write if cleared) before cert verify
 * - Scan ServerCert→FAIL_9 window for ALL call+test-eax gates → xor eax,eax at call-site
 * - Force local success jcc after each patched gate (je→jmp / jne-fail→nop)
 * - Observe FAIL_9 only (no mov→21); dump strHost+flag+BT RVAs
 * - Keep modulus@0x24b0 dumpCert; drop blind FAIL_9 jcc-NOP as primary hammer
 *
 * MODE=diag   — map gates + observe (no patches)
 * MODE=bypass — softHost sticky + gate xor (default)
 */
"use strict";
...
/** Map window: ServerCert through FAIL_9 + fuzzy-BT neighbors (v54). */
const RVA_GATE_SCAN = 0x612d000;
const GATE_SCAN_SIZE = 0x6000; // → 0x6133000
/** Only auto-patch gates in this tighter ServerCert band (avoid size-check je). */
...
function dumpBacktrace(ctx, tag) {
  try {
    ...
    dumpHostFlag(tag);
    // Disasm from exact fuzzy-BT RVAs (avoid walking into bad bytes before FAIL_9)
    const probeRvas = [0x612f38f, 0x61321de, 0x613222a, 0x6133638, 0x612d548, 0x612d5a2];
    for (let p = 0; p < probeRvas.length; p++) {
      disasmRange(mod, probeRvas[p], 0x10, 0x30, "probe+" + probeRvas[p].toString(16));
    }
    const hits = [];
    for (let i = 0; i < 80; i++) {
      try {
...
function findEaxTestGates(mod) {
  ...
  for (let i = 0; i < raw.length - 16; i++) {
    ...
    // Look for test eax,eax (85 c0) within 14 bytes after call
    for (let d = 5; d <= 14; d++) {
...
function classifyGate(g) {
  ...
  // DirtySDK: first gate in ServerCert ≈ hostname; later ≈ verify
...
/**
 * Should we patch this gate?
 * - Always log all gates in the map window.
 * - Patch when inside ServerCert band OR jcc is verify-style (js/jne fail),
 *   OR target is known 0x6129eb0 / historic site.
 * - Skip bare je outside band (size/empty checks: xor would take the empty path).
 */
function shouldPatchGate(g, callRva) {
  ...
  if (inBand && (k.indexOf("74") >= 0 || k.indexOf("84") >= 0)) return true; // hostname/verify je-success
  // Outside band: only verify-style (never blind je — size checks)
  if (!inBand && verifyStyle) return true;
  return false;
}
 ...
 * Then force success polarity on the following jcc.
...
function patchEaxTestGates(mod) {
  ...
  for (let i = 0; i < gates.length; i++) {
    ...
    // Never rewrite callee prologue of known-hot 0x6129eb0 — call-site only
    patchBytes(
      g.callAddr,
      XOR_EAX_NOP3,
  ...
  if (MODE === "bypass") {
    try {
      ...
      // Force je→jmp at historic site (+10 from call) if still 74
...
/**
 * Optional: if a gate target has ONLY one xref in the ProtoSSL window and is
 * NOT the known-hot RVA_REAL_CHECK, we could xor+ret its prologue. Skipped by
 * default — call-site xor is safer (v44 hang on attach to 0x6129eb0).
 */
function maybePatchUniqueVerifyPrologue(mod, gates) {
  // Intentionally no-op in v55 — keep call-site only. Logged for mapping.
  const counts = {};
  for (let i = 0; i < gates.length; i++) {
      ...
      onEnter(args) {
        ...
        console.log(
          "[ssl-bypass] ★ HIT FAIL_9 path #" +
            ...
            " (" +
            stateLabel(readState()) +
            ") — verify gates should have returned 0",
        );
...
function setupAll(tag) {
  ...
  console.log(
    "[ssl-bypass] setup " +
      ...
      " [" +
      tag +
      "] — eax-test gates → xor (no force/flush/inject/FAIL9→21)",
  );
  ...
  disasmRange(mod, RVA_FAIL_9, 0x8, 0x10, "FAIL_9-stub");

  const n = patchEaxTestGates(mod);
  maybePatchUniqueVerifyPrologue(mod, gatedTargets);

  if (MODE === "diag") {
    console.log("[ssl-bypass] DIAG: gates mapped, no patches applied");
  } else if (n === 0) {
    console.log("[ssl-bypass] WARN: no gates patched — bypass may fail");
  }

  setupDone = true;
  console.log("[ssl-bypass] setup done patches=" + patchCount + " gatesPatched=" + n + " mode=" + MODE);
}
...
function tryAdopt(addr, tag) {
  ...
  setFlagOnce(tag);
  // Host often empty at connect — softHost waits for CH; flag early is fine
  logState(tag);
  return true;
}
...
function hookRecv() {
  ...
  Interceptor.attach(addr, {
    onEnter(args) {
      ...
      // Re-assert host/flag BEFORE ProtoSSL processes this record (verify window)
...
console.log(
  "[ssl-bypass] loaded " +
    ...
    " MODE=" +
    MODE +
    " — eax-test gates→0 (no force/flush/inject)",
);
hookConnect();
hookSendFamily();
hookRecv();
