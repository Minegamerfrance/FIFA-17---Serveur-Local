/**
 * FIFA 17 ProtoSSL bypass v59 — Stalker autopsy (Gemini)
 *
 * v58 proven: FAIL_9 @ 0x61326fa is NOT fall-through.
 * Pre-stub is isolated switch/error case via jump table:
 *   mov edx,2 / mov rcx,rbp / lea r8d,[rdx+0x28] / call errorlog
 *   mov eax,0x1009 / jmp common-exit
 * Static XREF / fall-through je→jmp find nothing useful.
 *
 * v59 KEEP (no verify patches):
...
function enableFail9Stalker(mod, reason) {
  ...
  // Prefer events.block + onReceive (lighter); fall back to transform.callout
  try {
    Stalker.follow(tid, {
      events: { block: true, call: false, ret: false, exec: false, compile: false },
      onReceive: function (events) {
        try {
          parseStalkerEvents(events);
        } catch (_) {}
      },
    });
    stalkerMode = "events";
    ...
    console.log("[ssl-bypass] FAIL9-stalker events API fail: " + eEvents + " — trying transform");
    try {
      Stalker.follow(tid, {
      ...
      console.log(
        "[ssl-bypass] FAIL9-stalker ENABLE FAILED (game continues without stalker): " +
          eEvents +
          " / " +
          eTransform,
      );
...
function setupAll(tag) {
  ...
  console.log(
    "[ssl-bypass] setup " +
      ...
      " [" +
      tag +
      "] — softHost/flag + FAIL_9 observe + stalker autopsy (no xor/fallthrough)",
  );
