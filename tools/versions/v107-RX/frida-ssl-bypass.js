...
const VERSION = "v107-RX";
const STALKER_ON = false;
/** v103: Stalker crashes FIFA on ProtoSSL/send thread — keep OFF. */
const REPLY_STALKER_ENABLE = false;
/**
 * v106: after redirector, hijack ANY connect that isn't redirector/FUT/nucleus
 * → 127.0.0.1:10041 (game-owned socket = CAS A if it fires).
 */
const FORCE_BLAZE = true;
/** Never rewrite FUT :8000 (sends HTTP GET to Blaze — proven dead). */
...
/** v104/v105: force connect to Blaze from Frida timer (not Stalker). */
const FORCE_BLAZE_TCP = true;
/** v105: send captured ProtoSSL ClientHello (not plain 00 00 00 00). */
const FORCE_BLAZE_PROTOSSL = true;
/** v106: after XML, deep-poke IpAddress-like structs in RW memory. */
const INJECT_DEEP = true;
/**
 * v107: after XML, async LEA scan of all r-x pages (packed code lives there at runtime).
 * Finds getServerInstanceHttp / ServiceResolver / ProtoHttp consumers → hook them.
 */
const RX_LEA_ASYNC = true;
/** v107: Heat2 often uses memcmp not strcmp for tags. */
const HOOK_MEMCMP = true;
      ...
      onLeave(retval) {
        ...
        try {
          ...
          if (
            ...
            if (!sawRedirectorHttp) sawRedirectorHttp = true;
            try {
...
function armRxLeaHunt(reason) {
  ...
  function step() {
    ...
    const batchEnd = Math.min(ji + 4, jobs.length); // 4MB per tick
    for (; ji < batchEnd; ji++) {
      const job = jobs[ji];
      let u8;
      try {
...
function armResponseMarkerPoll(n, st) {
  ...
  setTimeout(function () {
    ...
    if (!sawRedirectorHttp) {
      ...
      try {
        if (INJECT_DEEP && plainBufStart) {
          deepInjectServerInstance(plainBufStart, plainBufLen || 400, "hold-release");
        }
      } catch (_) {}
      try {
        armRxLeaHunt("hold-release");
      } catch (_) {}
      scheduleForceBlazeTcp("hold-release");
      scheduleProofVerdict();
      proof(
...
console.log(
  "[ssl-bypass] ready — Cherche RX_LEA / GSI_CALL / MEMCMP / ★★★ BLAZE_CONNECT / VERDICT",
);
