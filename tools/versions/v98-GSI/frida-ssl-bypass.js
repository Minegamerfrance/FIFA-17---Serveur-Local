...
function armGsiConsumerHunt(reason) {
  ...
  // 2) MemoryAccessMonitor — first readers of the string bytes (packed binary gold)
...
function onTcpConnectAttempt(api, fd, sa, ctx) {
  ...
  // FORCE / INJECT disabled in v89-PROOF (observation only)
...
function armPlainConsumers(buf, len) {
  ...
  let page;
  try {
    page = buf.and(ptr("0xfffffffffffff000"));
  } catch (_) {
    page = buf;
  }
  const monSize = 0x2000;
  proof("MAM arm page@" + page + " watch buf@" + buf + " len=" + len);
  try {
    MemoryAccessMonitor.enable(
      [{ base: page, size: monSize }],
      {
        onAccess(details) {
          if (mamHits >= 10) return;
          try {
            if (details.operation !== "read") return;
            const a = details.address;
            if (a.compare(plainBufStart) < 0) return;
            if (a.compare(plainBufStart.add(plainBufLen)) >= 0) return;
            mamHits++;
            let sym = "";
            try {
              sym = String(DebugSymbol.fromAddress(details.from));
            } catch (_) {
              sym = String(details.from);
            }
            proof(
              "XML_READ #" +
                mamHits +
    ...
    setTimeout(function () {
      ...
      proof("MAM done hits=" + mamHits + (mamHits === 0 ? " (no read — copy elsewhere?)" : ""));
    }, 2500);
  } catch (e) {
    proof("MAM err " + e + " — rely on memcpy watch");
  }
}
...
function armMemcpyWatch() {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    Interceptor.attach(a, {
      onEnter(args) {
        if (!plainBufStart || copyHits >= 12) return;
        try {
          const dst = args[0];
          const src = args[1];
  ...
  proof("memcpy/memmove watch armed for XML buffer");
}
...
function armResponseMarkerPoll(n, st) {
  ...
  const times = [0, 2, 8, 20, 40, 80, 150, 250];
  ...
  setTimeout(function () {
    ...
    if (!sawRedirectorHttp) {
      ...
      scheduleProofVerdict();
      proof("SUMMARY after hold — peekDone=" + plainDumpDone);
    }
  }, HOLD_CLOSE_MS + 20);
}
...
hookDecodeDebug();
console.log(
  "[ssl-bypass] ready — Cherche [PROOF] GSI_ACCESS / GSI_CALL / ★★★ BLAZE / VERDICT",
);
