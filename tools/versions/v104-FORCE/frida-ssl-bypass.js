...
/** v104: from Frida JS timer (not Stalker), open TCP 127.0.0.1:10041 inside FIFA17.exe */
...
/**
 * v104: TCP probe to local Blaze from inside FIFA17.exe, via setTimeout
 * (Frida JS / gum thread — NOT Stalker, NOT ProtoSSL send path).
 * Does NOT count as CAS A (natural game connect).
 */
function scheduleForceBlazeTcp(reason) {
  if (!FORCE_BLAZE_TCP || forceBlazeDone) return;
  forceBlazeDone = true;
  proof("FORCE_BLAZE_TCP scheduled [" + reason + "] in 150ms");
  setTimeout(function () {
    forceBlazeTcpProbe(reason);
  }, 150);
}
...
function forceBlazeTcpProbe(reason) {
  ...
  try {
    ...
    // Tiny probe byte so server logs activity (not a valid Blaze frame)
    if (ok && sendFn && fd > 0) {
      try {
...
console.log(
  "[ssl-bypass] ready — Cherche FORCE_BLAZE_TCP / NUCLEUS_* / XML_BODY / ★★★ BLAZE(naturel) / VERDICT",
);
