...
/** OFF — blocking connect/recv from Frida freezes FIFA. */
const FORCE_BLAZE_TCP = false;
const FORCE_BLAZE_PROTOSSL = false;
/** OFF — Memory.scanSync on RW freezes / races peek. */
const INJECT_DEEP = false;
/** OFF — 813× r-x LEA scan freezes FIFA hard (v107/v107b). */
...
/** Hold redirector shutdown until peek done or deadline. */
let holdCloseForPeek = false;
let holdCloseDeadline = 0;
/** Shorter — long HOLD + block closesocket feels like a freeze. */
const HOLD_CLOSE_MS = 350;
...
function setupAll(tag) {
  ...
  // v107b: do NOT arm RX_LEA here — memory hammer during handshake → peek XML miss
}
...
console.log(
  "[ssl-bypass] ready — Cherche ★★★ BLAZE_CONNECT / FORCE_BLAZE_HIJACK / XML_BODY / VERDICT (no RX scan)",
);
