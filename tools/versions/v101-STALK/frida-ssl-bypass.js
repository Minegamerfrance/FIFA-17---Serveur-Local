...
/** v101: short Stalker on ProtoSSL/recv after HTTP reply (no MAM). */
const REPLY_STALKER_MS = 2200;
const REPLY_STALKER_MAX_CALLS = 60;
const REPLY_STALKER_RING = 24;
...
function dumpReplyStalkerAutopsy(reason) {
  ...
  // Top call targets inside FIFA17
  const entries = [];
  ...
  const top = entries.slice(0, REPLY_STALKER_MAX_CALLS);
  ...
  if (!top.length) proof("REPLY_CALL none (empty summary)");
...
function enableReplyStalker(reason) {
  ...
  if (replyStalkerActive && replyStalkerTid === tid) {
    proof("REPLY_STALK already on tid=" + tid + " [" + reason + "]");
    return;
  }
  if (replyStalkerActive && replyStalkerTid !== tid) {
  ...
  try {
    Stalker.follow(tid, {
      ...
      transform: function (iterator) {
        ...
        try {
          inMod =
            bbStart.compare(mod.base) >= 0 &&
            bbStart.compare(mod.base.add(mod.size)) < 0;
        } catch (_) {}
        // Prefer ProtoSSL band + nearby blaze-ish code; also sample all FIFA calls lightly
        let hot = false;
        try {
          const rva = bbStart.sub(mod.base).toInt32() >>> 0;
          hot =
            (rva >= 0x6100000 && rva < 0x6200000) ||
            (rva >= 0x6d00000 && rva < 0x6f00000) ||
            (rva >= 0x5800000 && rva < 0x5a00000);
        } catch (_) {}
        if (inMod && hot) {
          try {
            iterator.putCallout(function (context) {
    ...
    proof("REPLY_STALK ON tid=" + tid + " [" + reason + "] ms=" + REPLY_STALKER_MS);
    console.log("[ssl-bypass] REPLY_STALK ON tid=" + tid + " [" + reason + "]");
  } catch (e) {
    replyStalkerActive = false;
  ...
  try {
    Interceptor.attach(mod.base.add(RVA_PARENT), {
      onEnter(args) {
        ...
        // v101: follow PARENT thread during reply window (decrypt/parse often here)
        try {
          if (replyWindowOpen && sawHttpAppDataOut) {
...
hookDecodeDebug();
console.log(
  "[ssl-bypass] ready — Cherche REPLY_STALK / REPLY_CALL / XML_BODY / ★★★ BLAZE / VERDICT",
);
