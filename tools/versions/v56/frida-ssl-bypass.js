/**
 * FIFA 17 ProtoSSL bypass v56
 *
 * Gemini v56:
 * - FAIL_9 stub @ 0x61326fa (mov eax,0x1009) is in the state-machine dispatcher,
 *   ~20KB past the ServerCert parse band.
 * - v55 only xor'd eax-test gates in 0x612d480–0x612e000 (parser) → missed final
 *   hostname/CA verify that jumps to FAIL_9.
 * - Strategy: find who jumps to FAIL_9 (thorough jcc/jmp XREF) → near that jcc,
 *   find preceding call+test eax → xor eax,eax; nop×3 at the call-site.
 ...
 *
 * MODE=diag   — map XREFs/gates + observe (no patches)
 * MODE=bypass — softHost sticky + FAIL9-xref xor + gate belt (default)
 */
"use strict";
...
function findFail9Xrefs(mod) {
  ...
  // Fall-through: insn immediately before stub
  try {
    let a = mod.base.add(RVA_FAIL_9 - 0x40);
    ...
    if (prevAddr && prevInsn) {
      console.log(
        "[ssl-bypass] FAIL_9 fall-through prev @ FIFA+" +
          (prevAddr.sub(mod.base).toInt32() >>> 0).toString(16) +
          " " +
          prevInsn.toString(),
      );
    }
  } catch (e) {
    console.log("[ssl-bypass] FAIL_9 fall-through probe err " + e);
  }
...
/**
 * Gemini v56 hammer: for each FAIL_9 XREF, xor the preceding verify call.
 * If jcc-to-FAIL9 is je/jz (taken when eax==0), also NOP that jcc so xor doesn't
 * accidentally take the fail path.
 */
function patchFail9XrefVerifies(mod) {
  ...
  if (fail9Xrefs.length === 0) {
    ...
    console.log(
      "[ssl-bypass] Prefer XREF; fallback = near-stub eax-test gates + v55 parser belt.",
    );
    console.log(
      "[ssl-bypass] If FAIL_9 still hits: watch BT + consider iState/iLastError writes.",
    );
      ...
      onEnter(args) {
        ...
        console.log(
          "[ssl-bypass] ★ HIT FAIL_9 path #" +
            ...
            " (" +
            stateLabel(readState()) +
            ") — XREF verify xor should have prevented this",
        );
...
function setupAll(tag) {
  ...
  console.log(
    "[ssl-bypass] setup " +
      ...
      " [" +
      tag +
      "] — FAIL9 XREF→xor verify (no force/flush/inject/FAIL9→21)",
  );
  ...
  // 1) Primary Gemini v56: XREF → FAIL_9 → xor preceding verify call
  const xrefRes = patchFail9XrefVerifies(mod);

  // 2) Fallback / belt: near-stub eax-test gates (region v55 skipped)
  const nearN = patchNearFail9Gates(mod);

  // 3) Keep v55 parser-band gate xor
  const n = patchEaxTestGates(mod);
  maybePatchUniqueVerifyPrologue(mod, gatedTargets);

  if (MODE === "diag") {
    console.log("[ssl-bypass] DIAG: XREFs/gates mapped, no patches applied");
  } else if (xrefRes.xorPatches === 0 && nearN === 0 && n === 0) {
...
console.log(
  "[ssl-bypass] loaded " +
    ...
    " MODE=" +
    MODE +
    " — FAIL9 XREF→xor verify (no force/flush/inject)",
);
hookConnect();
hookSendFamily();
hookRecv();
hookClose();
console.log(
  "[ssl-bypass] ready — Cherche: XREF+xor + ★PARSE_OK + ★HelloDone + natural ★ ClientKeyExchange (hs=16)",
);
